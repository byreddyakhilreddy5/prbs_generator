	...
	always @(*) begin
		case (prbs_type)
			...
			for (i = 0; i < 8; i = i + 1) begin
				lfsr_masked = {lfsr_masked[29:0], lfsr_masked[6] ^ lfsr_masked[5]} ;
			end
			end
			2'b01: begin // PRBS9: use bits [8:0]
				lfsr_masked = lfsr & 31'h1FF;
			for (i = 0; i < 8; i = i + 1) begin
				lfsr_masked = {lfsr_masked[29:0], lfsr_masked[8] ^ lfsr_masked[4]} ;
			end
			end
			2'b10: begin // PRBS15: use bits [14:0]
				lfsr_masked = lfsr & 31'h7FFF;
			for (i = 0; i < 8; i = i + 1) begin
				lfsr_masked = {lfsr_masked[29:0], lfsr_masked[14] ^ lfsr_masked[13]} ;
			end
			end
			2'b11: begin // PRBS31: use all bits
				lfsr_masked = lfsr & 31'h7FFFFFFF;
			for (i = 0; i < 8; i = i + 1) begin
				lfsr_masked = {lfsr_masked[29:0], lfsr_masked[30] ^ lfsr_masked[27]} ;
			end
			end
		endcase
	end
